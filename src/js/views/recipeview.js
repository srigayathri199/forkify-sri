...
// console.log(icons); //which is icon file in dist
...
class RecipeView extends view {
    ...
    parentElement = document.querySelector('.recipe');
    //#errmessage
    errmessage = 'We could not find that recipe.Please try another one!';
    message = '';

    servingscontrol(handler) {
        this.parentElement.addEventListener('click', function(e) {
            const servingsbtn = e.target.closest('.btn--update-servings');
            if (!servingsbtn) return;
            const servings = +servingsbtn.dataset.updateTo;
            console.log(servings);
            console.log(servingsbtn);
